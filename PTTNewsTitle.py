...
#     with open("PTTNewsTitle.html", "wb") as file:#抓下原始html檔,存成local檔案
#         file.write(data)
...
with open("PTTNewsTitle.html", encoding="UTF-8") as file1:#做成Beautifull Soup物件樹
...
data = soup.find("div", class_="b-list-container action-bar-margin bbs-screen")
#再以.find_all()來搜尋該上層標籤下的所有text
lst = []
for row in data.find_all("div", class_="board-title"):
    lst.append(row.get_text(strip=True))
for i in lst:
    print(i)
